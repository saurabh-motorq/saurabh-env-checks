...
async function performVehiclesWithMultipleDevicesCountCheck(database, context, env)
{
    ...
    try{
        ...
        if (vehicleCount[0].count === 0) {
            ...
            await insertAlertIntoPg(env.name,'COSMOS','VEHICLES_WITH_MULTIPLE_DEVICES_COUNT', {vehicleCount: vehicleCount[0].count});
        }
    }
    catch(err){
        context.log(err);
        await insertAlertIntoPg(env.name,'COSMOS','VEHICLES_WITH_MULTIPLE_DEVICES_COUNT', {details:'Check Failed'});
    }
}
