...
async function performEnrollmentCheck(database, context, env)
{
    ...
    try{
        ...
        if (ratio >= config.enrollmentSuccessRatio) {
            ...
            await insertAlertIntoPg(env.name,'COSMOS','ENROLLMENT_OPERATION_CHECK',{enrollmentSuccessRatio: ratio});
        }
    }
    catch(err){
        context.log(err);
        await insertAlertIntoPg(env.name,'COSMOS','ENROLLMENT_OPERATION_CHECK', {details: 'Enrollment operationWorking Check Failed'});
    }
}
