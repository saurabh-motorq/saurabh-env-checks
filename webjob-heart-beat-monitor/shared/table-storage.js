...
async function getEntitiesByPartitionKey(tableName, partitionKey) {
    const query = new azureStorage.TableQuery().where('PartitionKey eq ?', partitionKey)
    let response = await tableService.queryEntitiesAsync(tableName, query, null);
    entries = response.entries.map(item => parseTableEntry(item))
    return entries;
}
...
module.exports = {
    initaliseTable, createTableEntry, upsertEntityIntoTableStore, getEntity, getDataFromTableEntry, getEntitiesByPartitionKey
}