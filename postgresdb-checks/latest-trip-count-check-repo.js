...
async function performLatestTripCountCheck(context,clientPgdb,env)
{
    ...
    try{
        ...
        if (tripCount.rows[0].count>0) {
            ...
            await pgRepo.insertAlertIntoPg(env.name,'POSTGRES','TRIPS_COUNT_IN_POSTGRES',{details: 'no trips found in the last 24 hours'});
        }
    }
    catch(err){
        context.log(err);
        await pgRepo.insertAlertIntoPg(env.name,'POSTGRES','TRIPS_COUNT_IN_POSTGRES', {details: 'Check Failed'});
    }
}
