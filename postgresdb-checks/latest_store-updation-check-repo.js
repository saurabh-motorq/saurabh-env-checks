...
async function performLatestStoreUpdationCheck(context,clientPgdb,env)
{
    ...
    try{
        ...
        if (latestUpdationCount.rows[0].count>0) {
            ...
            await pgRepo.insertAlertIntoPg(env.name,'POSTGRES','LATEST_STORE_UPDATION_IN_POSTGRES',{details:'no updates found in latest_store in the last 24 hours'});
        }
    }
    catch(err){
        context.log(err);
        await pgRepo.insertAlertIntoPg(env.name,'POSTGRES','LATEST_STORE_UPDATION_IN_POSTGRES', {details: 'Check Failed'});
    }
}
